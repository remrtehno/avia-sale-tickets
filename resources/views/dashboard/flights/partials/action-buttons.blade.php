<nobr>
  ...
  @php
    $isAsignedFlight = $assignedFlihts ?? false;
    $canBeDeleted = !$isAsignedFlight && $row?->canBeDeleted();
  @endphp

  @if (!Route::is('dashboard.top-flights.index'))
    <form method="POST"
      @if ($canBeDeleted) action="{{ route('dashboard.flights.destroy', ['flight' => $id]) }}" @endif
      class="d-inline-block">
      @csrf
      @method('delete')
      <button @if (!$canBeDeleted) disabled @endif onclick="return confirm('Уверены что хотите удалить?')"
